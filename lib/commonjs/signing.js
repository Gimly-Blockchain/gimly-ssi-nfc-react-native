...
class Sign {
  ...
  */
  ...
  static async documentLoader(url) {
    if (url.startsWith('did:')) {
      ...
      const framed = await (0, _jsonld.frame)('https://identity.foundation/EcdsaSecp256k1RecoverySignature2020/lds-ecdsa-secp256k1-recovery2020-0.0.jsonld', {
  ...
  */
  ...
  static async credential(unsignedCredential, keyId) {
  ...
  */
  ...
  static async presentation(presentation, keyId) {
    const suite = await this.getSuite(keyId);
  ...
  */
  ...
  static async verfyCredential(signedVC, keyId) {
    const suite = await this.getSuite(keyId);
