...
Object.defineProperty(exports, "EllipticCurve", {
  enumerable: true,
  get: function () {
    return _tangemSdk.EllipticCurve;
  }
});
exports.default = exports.NfcSdkModule = void 0;

var _tangemSdk = _interopRequireWildcard(require("tangem-sdk"));
...
const NfcSdkModule = _tangemSdk.default;
...
class NfcSdk {
  ...
  static async scanCard(initialMessage) {
    const data = await _tangemSdk.default.scanCard(initialMessage);
    const {
      cardId,
      batchId,
  ...
  static async createKey(curve) {
    ...
    let ellipticCurve = _tangemSdk.EllipticCurve.Secp256k1; //default

    if (curve === 'secp256k1') ellipticCurve = _tangemSdk.EllipticCurve.Secp256k1;
    if (curve === 'ed25519') ellipticCurve = _tangemSdk.EllipticCurve.Ed25519;
    if (curve === 'secp256r1') ellipticCurve = _tangemSdk.EllipticCurve.Secp256r1;
    const data = await _tangemSdk.default.createWallet(ellipticCurve, NfcSdk.cardId);
    const keyInfo = {
  ...
  static async deactivateKey(keyId) {
    ...
    const response = await _tangemSdk.default.purgeWallet(keyId || NfcSdk.keyId, NfcSdk.cardId);
    return response;
  }
  /**
  ...
  static async signUsingKey(signRequest, keyId) {
    ...
    const data = await _tangemSdk.default.signHashes(hashes, keyId || NfcSdk.keyId, NfcSdk.cardId); // @ts-ignore
  ...
  static async signCredential(signCredentialRequest, keyId) {
    ...
    const signedCredential = await _signing.default.credential(signCredentialRequest.credential, keyId || NfcSdk.keyId, NfcSdk.cardId, signCredentialRequest.controller);
  ...
  static async verifyCredential(signCredentialRequest) {
    const verification = await _signing.default.verifyCredential(signCredentialRequest.credential, NfcSdk.keyId, NfcSdk.cardId, signCredentialRequest.controller);
  ...
  static async signPresentation(signPresentationRequest, keyId) {
    ...
    const signedPresentation = await _signing.default.presentation(signPresentationRequest.presentation, keyId || NfcSdk.keyId, NfcSdk.cardId, signPresentationRequest.controller, signPresentationRequest.challenge);
  ...
  static async verifyPresentation(signPresentationRequest) {
    const verification = await _signing.default.verifyPresentation(signPresentationRequest.presentation, NfcSdk.keyId, NfcSdk.cardId, signPresentationRequest.controller, signPresentationRequest.challenge);
    return verification.verified;
  }
  /**
   * Store a credential
  ...
  static async storeCredential(credential, fileName) {
    ...
    const files = await _tangemSdk.default.readFiles(true); // @ts-ignore
    ...
    const {
      startingSignature,
      finalizingSignature
    } = await _tangemSdk.default.prepareHashes(NfcSdk.cardId, data, fileCounter, fileName, '11121314151617184771ED81F2BACF57479E4735EB1405083927372D40DA9E92' // TODO hardcoded key due to a Tangem SDK limitation
    );
    const file = {
      startingSignature,
      finalizingSignature,
      ...
      fileName,
      counter: fileCounter,
      fileVisibility: _tangemSdk.FileVisibility.Public
    };
    const response = await _tangemSdk.default.writeFiles([file], NfcSdk.cardId);
    return response;
  }
  /**
  ...
  static async deleteStoredCredential(fileName) {
    const files = await _tangemSdk.default.readFiles(true, fileName); // @ts-ignore

    const indices = files.map(file => file.index);
    await _tangemSdk.default.deleteFiles(indices);
    const response = {
  ...
  static async getStoredCredentials() {
    const files = await _tangemSdk.default.readFiles(true);
  ...
  static async getStoredCredential(fileName) {
    const files = await _tangemSdk.default.readFiles(true, fileName);
    const credentials = files // @ts-ignore
    .map(item => {
      try {
        const decodeData = hexDecode(item.data);
  ...
  static async setAccessCode(accessCode) {
    ...
    const data = await _tangemSdk.default.setAccessCode(accessCode, NfcSdk.cardId);
    return data;
  }
  /**
  ...
  static async setPasscode(passcode) {
    ...
    const data = await _tangemSdk.default.setPasscode(passcode, NfcSdk.cardId);
    return data;
  }
  /**
  ...
  static async resetUserCodes() {
    ...
    const data = await _tangemSdk.default.resetUserCodes(NfcSdk.cardId);
    return data;
  }
  /**
