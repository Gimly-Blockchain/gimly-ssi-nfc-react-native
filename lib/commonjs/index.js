...
class NfcSdk {
  ...
  static async createKey(cardId, curve) {
    ...
    const data = await _tangemSdkReactNative.default.createWallet(curve, cardId); // data = {"cardId":"AC79000000000004","wallet":{"totalSignedHashes":0,"index":8,"hasBackup":false,"derivedKeys":[],"curve":"secp256k1","publicKey":"037C58B433A9063575C12232A0763DCC09423C8180E977AE9F8C527F462DBC4B5C","chainCode":"CB2C5D06D739E6D42321A8E456B3DF2E70308ECE75DF6EAE830D4D0B2B3F887B","settings":{"isPermanent":false}}}
  ...
  static async deactivateKey(cardId, keyId) {
    ...
    const data = await _tangemSdkReactNative.default.purgeWallet(keyId, cardId); // data = {cardId: 'AC79000000000004'}
  ...
  static async getKeys(initialMessage, //TODO the initialMessage key is not defined on the Terminal API
  cardId) {
    ...
    const keys = data.wallets ? data.wallets.map(wallet => {
      const keyInfo = {
        ...
        index: wallet.index // status: '??', ---> TODO: we're not using this prop here and the terminal api
  ...
  static async getKey(initialMessage, cardId, keyId) {
    ...
    const keyInfo = {
      ...
      index: filtered[0].index // status: '??', ---> TODO: we're not using this prop here and the terminal api
  ...
  static async signUsingKey(keyId, signRequest, cardId) {
    ...
    const data = await _tangemSdkReactNative.default.signHashes(hashes, keyId, cardId); // data = {"totalSignedHashes":12,"cardId":"AC79000000000004","signatures":["64B4BB5428A78605E2E1AD9FE1F7CDFE336094C8F1A2D0583D16C6591CACCB7775E142B0B32D8DC19868892DC9EB2EC50CE8657C24AB79A97B45A535A6455958","86F2980929F858170F89D39F9E3D07D8ABEC6F5FFAD9692D2FBF4725C95F091C14EEF455BC559F2BB76ECC9BCB6E90CB93C305D7D4ABAE3195D0463B6D5E9182"]}
  ...
  static async signCredential(keyId, signCredentialRequest, cardId) {
    ...
    console.log(keyId, signCredentialRequest, cardId); // TODO: hadcoded VC
    ...
    try {
      const requestResponse = await _tangemSdkReactNative.default.writeOwnerFile(files, keyId, cardId); // TODO: check response to convert to desired format response
  ...
  static async deleteStoredCredential(credentialId, cardId) {
    ...
    return await _tangemSdkReactNative.default.deleteFiles(indicesToDelete, cardId).then(response => {
      return response;
    }).catch(() => {
      return null;
    });
  ...
  static async setAccessCode(accessCode, cardId) {
    return _tangemSdkReactNative.default.setAccessCode(accessCode, cardId).then(result => {
      return result;
    }).catch(() => {
      return null;
    });
  ...
  static async setPasscode(passcode, cardId) {
    return _tangemSdkReactNative.default.setPasscode(passcode, cardId).then(result => {
      return result;
    }).catch(() => {
      return null;
    });
  ...
  static async resetUserCodes(cardId) {
    return _tangemSdkReactNative.default.resetUserCodes(cardId).then(result => {
      return result;
    }).catch(() => {
      return null;
    });
